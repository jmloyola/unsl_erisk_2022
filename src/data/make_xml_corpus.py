...
def generate_raw_gambling_corpus(replace_old=True):
    ...
    corpus_dir = os.path.join(
        raw_corpus_path, "t1_training", "TRAINING_DATA", "2021_cases"
    )
    ...
    if os.path.isfile(output_file_path):
        if replace_old:
            print_message(
                f"Cleaning the corpus {output_file_name} previously generated."
            )
            os.remove(output_file_path)
        else:
            print_message(
                f"The corpus {output_file_name} already exists. Delete it before running the script or call "
...
def generate_raw_depression_corpus(replace_old=True):
    ...
    print_message(
        f"Generating the corpus {output_file_name_train} using the data from 2017 ..."
    )
    ...
    if os.path.isfile(output_file_path_train):
        if replace_old:
            print_message(
                f"Cleaning the corpus {output_file_name_train} previously generated."
            )
            os.remove(output_file_path_train)
        else:
            print_message(
                f"The corpus {output_file_name_train} already exists. Delete it before running the script or "
    ...
    print_message(
        f"Generating the corpus {output_file_name_test} using the data from 2018 ..."
    )

    if os.path.isfile(output_file_path_test):
        if replace_old:
            print_message(
                f"Cleaning the corpus {output_file_name_test} previously generated."
            )
            os.remove(output_file_path_test)
        else:
            print_message(
                f"The corpus {output_file_name_test} already exists. Delete it before running the script or "
                "call this function with the option `replace_old=True`."
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build corpus from the xml files provided by the eRisk organizers."
    )
    parser.add_argument(
