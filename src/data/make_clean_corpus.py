...
def get_cleaned_post(post):
    ...
    clean_post = SUB_REDDIT_REGEX.sub(
        repl="\g<subreddit>", string=clean_post  # noqa: W605
    )
...
def generate_clean_corpus(corpus_name, corpus_kind, replace_old=True):
    ...
    for stage in ["train", "test"]:
        input_corpus_path = os.path.join(
            interim_corpus_path, f"{corpus_name}-{stage}-raw.txt"
        )
        ...
        if os.path.isfile(output_file_path):
            if replace_old:
                print_message(
                    f"Cleaning the corpus {output_file_name} previously created."
                )
                os.remove(output_file_path)
            else:
                print_message(
                    f"The corpus {output_file_name} already exists. Delete it beforehand or "
...
def generate_csv_truncated_corpus(corpus_name, corpus_kind, replace_old=True):
    ...
    replace_old : bool, default=True
        ...
        truncated corpus.
    """
    partial_input_output_path = os.path.join(
        PATH_INTERIM_CORPUS, corpus_kind, corpus_name
    )
    input_file_path_train = os.path.join(
        partial_input_output_path, f"{corpus_name}-train-raw.txt"
    )
    input_file_path_test = os.path.join(
        partial_input_output_path, f"{corpus_name}-test-raw.txt"
    )

    output_train_file_name = f"{corpus_name}_truncated_train.csv"
    output_train_file_path = os.path.join(
        partial_input_output_path, output_train_file_name
    )
    output_test_file_name = f"{corpus_name}_truncated_test.csv"
    output_test_file_path = os.path.join(
        partial_input_output_path, output_test_file_name
    )

    print_message(
        f"Creating the datasets {output_train_file_name} and {output_test_file_name}."
    )
    ...
    if os.path.isfile(output_train_file_path):
        if replace_old:
            ...
            print_message(
                f"The datasets {output_train_file_name} and {output_test_file_name} already exist. Delete "
...
if __name__ == "__main__":
    ...
    generate_clean_corpus(
        corpus_name=args.corpus, corpus_kind=args.kind, replace_old=False
    )
    if args.kind == "reddit":
        generate_csv_truncated_corpus(
            corpus_name=args.corpus, corpus_kind=args.kind, replace_old=False
        )
